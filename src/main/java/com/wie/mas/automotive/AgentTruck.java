...
public class AgentTruck extends Agent {
	...
	public void createAgent() {
		...
		if ((System.currentTimeMillis() - t0) > 10000 && a < 5) {
			String name = "T" + a;
			...
			try {
				AgentController at = c.createNewAgent(name, "com.wie.mas.automotive.AgentTruck", null);
				at.start();
				a++;
				System.out.println(alice.getLocalName() + " Created");

				ACLMessage s_msg = new ACLMessage(ACLMessage.INFORM);
				s_msg.setContent("ping_weight_90");
				...
				if (al.contains("R"))
					...
					System.out.println("Truck Recharge Station not found!!!!!!");
	...
	class B2 extends OneShotBehaviour {
		...
		private void createAgent2() {
			...
			if ((System.currentTimeMillis() - t0) > 20000 && c <= 1) {

				ConverterUtils.DataSource source1;
				try {
					source1 = new ConverterUtils.DataSource("ev_truck_train.arff");
					...
					ConverterUtils.DataSource source2 = new ConverterUtils.DataSource("ev_truck_test.arff");
				...
				while (itr.hasNext()) {
					...
					try {
						AgentController a = c.createNewAgent(name, "com.wie.mas.automotive.AgentTruck", null);
						a.start();
						System.out.println(alice.getLocalName() + " Created");
